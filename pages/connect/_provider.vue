<template>
  <div class="flex items-center justify-center h-screen">
    <svg role="status" class="inline w-28 h-28 text-gray-200 animate-spin fill-cyan-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  async mounted() {
    ...
    const userRes = await this.$axios.$get(`/users/${user.id}/?populate=%2A`, {
      headers: headers
    });
    ...
    if (!userRes.name || !userRes.firstname || !userRes.role || !userRes.pole) {
      ...
      if (userRes.role.name !== 'Chef de projet') {
        this.$router.push(`/calendrier/${userRes.role.name.toLowerCase()}`)
      } else if (userRes.role.name === 'Chef de projet') {
